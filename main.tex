...
Как известно, согласно [1] Адамару, задача о нахождении решения уравнения
\begin{equation}
    \label{eq:auf}
    Au = f
\end{equation}
...
Это, в соответствии с [2], стр.58, означает:
\begin{enumerate}
...
\end{equation}
Если полугруппа U(t, -A) удовлетворяет условиям 1---5, то по Балакришнану [2], стр.358, для оператора $A$ определена
...
\end{equation}
Тогда, согласно Балакришнану [2], стр.358, если $A$-- $B$--позитивный, то определена дробная степень $A^{-\alpha}$:
\begin{equation}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{aligned}
    \end{equation}
    где $h_{t, \alpha}(\xi) > 0$ --- функция Иосиды со свойствами [2], стр.358
    \begin{equation}
        \label{eq:one_int}
    ...
    Таким образом операторы $-A_1^{\alpha_1}$ и $-A_2^{\alpha_2}$ удовлетворяют теореме Да Пратто, Грисварда [3]
    об обращении суммы операторов
    \begin{equation}
        Lu = (A_1^{\alpha_1} + A_2^{\alpha_2})u = f
    \end{equation}
    и пользуясь представлением ([4])
    \begin{equation}
...
Приведенные в этом разделе полугруппы относятся по [6] к классу ''канонических полугрупп'' и в соответствии
с [4] определяются следующим образом:
...
\begin{enumerate}
    ...
    Дробным степеням операторов $\pm x \frac{d}{dx}$ соответствуют дробные производные Адамара [7], стр.251.
    ...
    Такие полугруппы, в соответствии с [6] называются гиперболическими.
\end{enumerate}

{\bf Заключение.} В заключении заметим, что теорема 1 аналогичными рассуждениями распространяется на случай конечного числа слагаемых с операторами $A_m^{\alpha_m}$, $\alpha_m\in(0,1)$, $\|U(t_1-A_m)\|\le M_me^{-\omega_mt}$. В этом случае уравнение $Lu=\sum\limits_{m=1}^nA_m^{\alpha_m}u=f$ имеет решение $u=\int\limits_0^\infty\prod\limits_{m=1}^nU(t_1-A_m)fdt$, с оценкой $\|u\|_E\le\frac{\prod_{m=1}^nM_m}{\prod_{m=1}^n\omega_m\alpha_m}\|f\|_E$.

{\bf Литература}
[1] Крейн С.Г. Линейные дифференциальные уравнения в банаховом пространстве/ С.Г. Крейн.-- М.: Наука, 1967.--- 464 с.

[2] Иосида К. Функциональный анализ: Учебник/ К. Иосида, пер. с анг. В.М. Волосова. -- М.: Мир, 1967--- 624 с.

[3] Тихонов А.Н. Уравнения математической физики/ А.Н. Тихонов, А.А. Самарский.-- М.: Наука.-- 1966.--- 724 с.

[4] Костин, В. А. Элементарные полугруппы преобразований и их производящие уравнения / В. А. Костин, А. В. Костин, Д. В. Костин // Доклады Академии наук.--- 2014. – Т. 455, № 2. – С. 142-146.

[5] Da Prato  Giuseppe / Giuseppe Da Prato, Pierre Grisvard.-- Paris : Gauthier-Villars.-- 1975. Journal de mathematiques pures et appliquees; serie 9, tome 54, fasc. 3.-- pp. 305-387.

[6] Хилле Э. Функциональный анализ и полугруппы /Э. Хилле, Р. Филлипс М.: Изд-во иностр. лит., 1962. 829 с.

[7] Самко С.Г. Интегралы и производные дробного порядка и некоторые их приложения / С.Г. Самко, А.А. Килбас, О.И.Маричев.--- Минск: Наука и техника, 1987.--- 687 с.
